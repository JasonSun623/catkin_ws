...
namespace SickToolbox {
  ...
  {
      arg=new std::string[5000];//chq
      argumentcount_=0;//chq
   ...
   */
  void SickLMS1xx::GetSickMeasurements(
          unsigned int * const range_1_vals,
          ...
          unsigned int & freq_scan,
                   int & measure_start_angle,
          unsigned int & angle_resolution,
          unsigned int & measure_start_time,
    ...
    int count = 0;
    //std::cout<<"SickLMS1xx::GetSickMeasurements.argumentcount_: "<< argumentcount_ <<std::endl;
    if (argumentcount_ < 6){
        std::cout<<"SickLMS1xx::GetSickMeasurements.Scan data unsuccessful.no enough data !"<<std::endl;
        return;
    }

    //if (argumentcount_ < 5 || atoi( (arg[5]).c_str() ) != 0 ) // 0 no error
    //{
    //    std::cout<<"SickLMS1xx::GetSickMeasurements.Scan data unsuccessful.arg[5]: " << arg[5]<<std::endl;
    //    return;
    //}
    ...
    if (range_1_vals != NULL) {
      ...
      substr_dist_1_pos = 0;
      while( substr_dist_1_pos < argumentcount_ ){

          if( arg[substr_dist_1_pos] == substr_dist_1 )break;
          else substr_dist_1_pos++;
      }
      if(  substr_dist_1_pos >= argumentcount_ ){

          throw SickIOException("SickLMS1xx::GetSickMeasurements: _find Sub String() failed!");
      }
      ...
      if(argumentcount_ < 9 ){
          throw SickIOException("SickLMS1xx::GetSickMeasurements: time stamp lost.failed!");
      }

      /* Extract scan  start time */
      measure_start_time = _ConvertHexToUInt(arg[9]);
      //std::cout << "lms 111 measure_start_time:  " << measure_start_time << std::endl;

      /* Extract scan  frequency */
      freq_scan = _ConvertHexToUInt(arg[16]);
     // std::cout << "lms 111  freq_scan:  " << freq_scan << std::endl;


      /* Extract Num DIST1 Values */
      //payload_str = (char *)&payload_buffer[substr_dist_1_pos];

     //jump 2
      //for (unsigned int i = 0; i < 2; i++) {
      //payload_str = _convertNextTokenToUInt(payload_str,null_int);
      //std::cout << "null_int:  " << null_int << std::endl;

      //}
      /* Grab the scan start angle  */
      //printf("lms 111  start_angle:%s.\n",*payload_str);
      //payload_str = _convertNextTokenToUInt(payload_str,*start_angle);

      //std::cout << " substr_dist_1_pos : " << substr_dist_1_pos << std::endl;

      measure_start_angle = _ConvertHexToDec(arg[substr_dist_1_pos+3]);
       //std::cout << "lms 111  measure_start_angle:  " << measure_start_angle << std::endl;
      /* Grab the scan angle resolution   */
       //payload_str = _convertNextTokenToUInt(payload_str,*angle_resolution);
       angle_resolution =  _ConvertHexToUInt(arg[substr_dist_1_pos+4]);
       // std::cout << "lms 111  angle_resolution:  " << angle_resolution << std::endl;
      /* Grab the scan num */
      //payload_str = _convertNextTokenToUInt(payload_str,num_dist_1_vals);
        num_dist_1_vals = _ConvertHexToUInt(arg[substr_dist_1_pos+5]);
      //std::cout << "lms 111  num_dist_1_vals:  " << num_dist_1_vals << std::endl;

      /* Grab the DIST1 values */
      //std::cout << "SickLMS1xx::GetSickMeasurements.range dist:\n ";
      for (unsigned int i = 0; i < num_dist_1_vals; i++) {
     // payload_str = _convertNextTokenToUInt(payload_str,range_1_vals[i]);
       
        range_1_vals[i] = _ConvertHexToUInt(arg[substr_dist_1_pos+6+i]);
        //if( i == 10 )std::cout << "get sick measurement .angle(deg): "<< (float) i * 0.5<< " dist: " << range_1_vals[i] << "\n ";
      }
      //std::cout << std::endl;
    }
  ...
  int SickLMS1xx::_ConvertHexToDec(std::string num)
  {
      int suma=0;
      ...
      if( num[0] == 'F' || num[0] > 65){
          ...
          printf( "convert str to num.str: %s,conv num: %d\n",  num.c_str(),suma);
      }
      return suma;
  ...
  void SickLMS1xx::_SplitReceivedMessage(SickLMS1xxMessage recv_message)
  {
      std::string str="";
      argumentcount_=0;
      ...
      for (int i=0;i<messagelength;i++)
      {
          ...
          if (message[i]==' ' || message[i] == 0x03 )//03 means end add by chq 2018.7
          {
              ...
              //std::cout << " SickLMS1xx::SplitReceivedMessage.arg[" << argumentcount_ << "]:"<< arg[argumentcount_] <<str << " "<<std::endl;
              argumentcount_++;
              str="";
              continue;
          }
  ...
  char * SickLMS1xx::_convertNextTokenToUInt( char * const str_buffer, unsigned int & num_val,
    ...
    const char * token = NULL;
    uint32_t curr_val = 0;
    if ((token = strtok(str_buffer,delimeter)) == NULL) {//ru 如果没找到分隔符，找到了返回匹配到的字符串指针 chq
      throw SickIOException("SickLMS1xx::_getextTokenAsUInt: strtok() failed!");
    }

    if (sscanf(token,"%x",&curr_val) == EOF) {//从字符串token中按格式传输一个数据到curr_val,返回EOF代表有错误或者到达字符串末尾 ，这里代表将字符串输出为十六进制数据chq
  ...
  unsigned int SickLMS1xx::_ConvertHexToUInt(std::string num){
    uint32_t curr_val = 0;
    unsigned int num_val;
    if (sscanf(num.c_str(),"%x",&curr_val) == EOF) {//从字符串token中按格式传输一个数据到curr_val,返回EOF代表有错误或者到达字符串末尾 ，这里代表将字符串输出为十六进制数据chq
      throw SickIOException("SickLMS1xx::_ConvertHexToUInt: sscanf() failed!");
    }
