...
pf_slam::pf_slam(){
  ...
  private_nh.param<double>("keep_to_otherrfs_dist",keep_to_otherrfs_dist,2.5);
  private_nh.param<double>("keep_to_otherrfs_angle_deg",keep_to_otherrfs_angle_deg,10);
...
void pf_slam::callBackMappingStop(const std_msgs::String fname){
  ...
  if (!out)
  {
    ROS_ERROR(" pf_slam::callBackMappingStop.Couldn't save map file to %s.return!!!", str_name.c_str());
    return;
  }
  mapping_start = false;
  ROS_INFO("pf_slam.saving map start...");
  ///1 out history_rfs
  out<<"[landmarks]\n";
...
void pf_slam::getNewRfs(geometry_msgs::Pose2D scan_pos, std::vector<VecPosition>& mea_rfs){
 ...
 ROS_INFO("pf_slam::getNewRfs. cur_candidate_match_rfs size is:%d...",cur_candidate_match_rfs.size());
// cur_candidate_match_rfs = cur_abs_rfs;
 ...
 for(int i = 0; i < mea_size; i++){
   ...
   for(int j = 0; j < cand_size; j++){
     ...
     ROS_INFO("pf_slam::getNewRfs.cur mea index:%d,candi map rfs index:%d,com_dist,angle:(%.6f,%.6f)",i,cur_candidate_match_rfs[j],cmp_dist[i][j],cmp_angle[i][j]);
     dis_match = cmp_dist[i][j] <= keep_to_otherrfs_dist;
     ang_match = cmp_angle[i][j] <= keep_to_otherrfs_angle_deg;
     if( dis_match ){
        is_new_rfs = false;
        break;
      }
    }
    if( is_new_rfs ){//no cur measured rfs in map rfs
      VecPosition abs_mea_pos = cur_mea_rfs[i];
      abs_mea_pos.rotate(Rad2Deg(scan_pos.theta));
      abs_mea_pos += v_scan_pos;
      ROS_INFO("pf_slam::getNewRfs.no matched map rfs to cur mea rfs.abs_mea_pos:(%.4f,%.4f).abs scan_pos:(%.4f,%.4f)",abs_mea_pos.getX(),abs_mea_pos.getY(), v_scan_pos.getX(),v_scan_pos.getY() );
      for(int k = 0; k < cand_size; k++ ){
        rel_rfs = cur_abs_rfs[cur_candidate_match_rfs[k]];
        ROS_INFO("pf_slam::getNewRfs.comp abs_map_pos(%.4f,%.4f).dist:%.4f,angle(deg):%.4f",rel_rfs.getX(),rel_rfs.getY(),cmp_dist[i][k],cmp_angle[i][k]);
      }
      mea_rfs.push_back(cur_mea_rfs[i]);

      ROS_INFO("pf_slam::getNewMatchedRfs.newrfs index:%d,rel pos:(%.3f,%.3f)!",i,cur_mea_rfs[i].getX(),cur_mea_rfs[i].getY());
    }
...
void pf_slam::calGlobalPosThread(){
  ...
  if(!mapping_start){
    ROS_ERROR("wait for running.pf_slam do not start cal GlobalPos Thread");
    return;
  }
 ...
 /// getMatchedMeaRfs(loc_pos,mea_rfs,matched_mea_rfs);
  ...
  for(int i = 0 ; i < matched_mea_rfs.size();i++){
    ...
    {//if do not meet upate condition,only to add cur pos to his data
      ...
      //judge the less size
      ROS_INFO("pf_slam.update his rfs.cur map rfs index:%d,after filter by history_rfs_reject_dist:%.3f, the less his rfs size:%d", map_rfs_index,history_rfs_reject_dist,filter_his_rfs.size());
      ...
      {//if too little,just update the data but not update the new avg pos
        ...
        ROS_INFO("------pf_slam.updatehisrfs------.rfs index:%d.cur real time avg dx:%.6f. record best dx:%.6f",map_rfs_index,var,cur_his_rfs_cord->dx());
        cur_his_rfs_cord->setCurDx(var);//record cur newest dx
        double cur_dx = cur_his_rfs_cord->dx();
        double delta_dx = cur_dx - var;
        ROS_INFO("pf_slam.updatehisrfs.now update rfs index:%d.firstly we cal the new his avg:%.6f. cur rfs dx:%.6f",map_rfs_index,var,cur_dx);
        if( delta_dx > his_rfs_update_thread*his_rfs_update_thread ){
         ...
         {
          boost::mutex::scoped_lock l(addrfs_mut);
          ROS_INFO("pf_slam.updatehisrfs.now update rfs index:%d.new avg pos:(%.6f,%.6f)",map_rfs_index,new_avg.getX(),new_avg.getY());
