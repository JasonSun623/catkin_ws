...
namespace robotino_local_planner
{
  ...
  void RobotinoLocalPlanner::initialize( 
    std::string name,
    tf::TransformListener* tf, 
    costmap_2d::Costmap2DROS* costmap_ros )
  {
    tf_ = tf;
    //added by  chq : "~" means there are privated paras 
    ...
    //added by chq 
    //ROS_INFO("RobotinoLocalPlanner initialize.heading_lookahead:%f,xy_goal_tolerance:%f",heading_lookahead_,xy_goal_tolerance_);
  }
  ...
  bool RobotinoLocalPlanner::computeVelocityCommands( geometry_msgs::Twist& cmd_vel)
  {
    ...
    ROS_INFO("RobotinoLocalPlanner::computeVelocityCommands.next_heading_index_:%d,state(0=rottostart,1=mov,2=rottogoal,3=finished):%d",next_heading_index_,state_);
    switch(state_)
    {
    ...
    ROS_INFO("RobotinoLocalPlanner::computeVelocityCommands.vx:%f,vy:%f,w:%f,state(0=rottostart,1=mov,2=rottogoal,3=finished):%d",cmd_vel.linear.x,cmd_vel.linear.y,cmd_vel.angular.z,state_);
  ...
  bool RobotinoLocalPlanner::setPlan( const std::vector<geometry_msgs::PoseStamped>& global_plan )
  {
    ...
    ROS_INFO_STREAM("RobotinoLocalPlanner.Get Global plan size: " << global_plan_.size() );
  ...
  bool RobotinoLocalPlanner::rotateToStart( geometry_msgs::Twist& cmd_vel )
  {
    ...
    {
      boost::mutex::scoped_lock lock(odom_lock_);
      ROS_INFO_STREAM("RobotinoLocalPlanner.rotateToStart:.waitForTransform.base_odom_.header.frame_id:" << base_odom_.header.frame_id
                      << "global_plan_[next_heading_index_].header.frame_id:" << global_plan_[next_heading_index_].header.frame_id);//chq for log
      ...
      //added by chq transfer global plan frame to odom frame so we can cal rlative dist and orientation
  ...
  bool RobotinoLocalPlanner::move( geometry_msgs::Twist& cmd_vel )
  {
    ...
    if( next_heading_index_ == (int) global_plan_.size()-1)
    {
      ...
      {
        ROS_INFO("RobotinoLocalPlanner::move.first reached to goal. cur dist to goal:%f",distance_to_next_heading);
        cmd_vel.linear.x = 0.0;
        cmd_vel.angular.z = 0.0;
        //state_ = RotatingToGoal;
        state_ = Finished;//chq add 到达目标点不转向
        return true;
      }
    }
    return true;
  ...
  void RobotinoLocalPlanner::computeNextHeadingIndex()
  {
    ...
    for( unsigned int i = curr_heading_index_; i < global_plan_.size() - 1; ++i )
    {
      ...
      {
          ROS_INFO_STREAM("RobotinoLocalPlanner.rotateToStart:.waitForTransform.base_odom_.header.frame_id:" << base_odom_.header.frame_id
                          << "global_plan_[i].header.frame_id:" << global_plan_[i].header.frame_id);//chq for log
      ...
      //added by chq 
      // if the dist bet curpos to path[i] below heading_lookahed para then we trace path[i]
      // otherwise continue cal next dist  
    ...
    //added by chq if all the dist above heading_lookahed para then we had to trace the last path point 
  ...
  double RobotinoLocalPlanner::calLinearVel()
  {
    ...
    //add by chq 偏差越大，vel越小，代表可能十个点之间弯曲（方向相对变化率）越大
    ...
    if( vel < min_linear_vel_ )
      vel = min_linear_vel_;
    ROS_INFO("RobotinoLocalPlanner::calLinearVel.cur vel:%f,diff_dist:%f,max_v:%f,min_v:%f",vel,diff,max_linear_vel_,min_linear_vel_);
    return vel;
  }
