 ...
 */
inline uint16_t host_to_sick_nav350_byte_order( uint16_t value ) {
 ...
 * \return Value in Sick LD byte order (big-endian)
 */
inline uint32_t host_to_sick_nav350_byte_order( uint32_t value ) {
 ...
 */
inline uint16_t sick_nav350_to_host_byte_order( uint16_t value ) {
  return REVERSE_BYTE_ORDER_16(value);
}
 ...
 * \return Value in host byte order (little-endian)
 */
inline uint32_t sick_nav350_to_host_byte_order( uint32_t value ) {
  return REVERSE_BYTE_ORDER_32(value);
}
