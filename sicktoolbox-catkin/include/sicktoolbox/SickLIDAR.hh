...
namespace SickToolbox {
  ...
  protected:
    ...
    bool _recvMessage( SICK_MSG_CLASS &sick_message, const unsigned int timeout_value ) const throw ( SickTimeoutException );//chq modify return type

    /** Search the stream for a payload with a particular "header" byte string */
    bool _recvMessage( SICK_MSG_CLASS &sick_message,
   ...
   */
  template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
  bool SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_recvMessage( SICK_MSG_CLASS &sick_message,
								      const unsigned int timeout_value ) const throw ( SickTimeoutException ) {
    ...
    while(!_sick_buffer_monitor->GetNextMessageFromMonitor(sick_message)) {    
      ...
      if (_computeElapsedTime(beg_time,end_time) > timeout_value) {
          ...
          std::cerr<< "SickLIDAR::_recvMessage: Timeout occurred!" << std::endl;
            return false;
           //throw SickTimeoutException("SickLIDAR::_recvMessage: Timeout occurred!");//chq disable
      }
   ...
   */
  template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
  bool SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_recvMessage(
    ...
    for(;;) {
      ...
      unsigned int i = 0;
      //std::cout << "_recvMessage .try to GetNextMessageFromMonitor..." <<std::endl;///--///
      if (_sick_buffer_monitor->GetNextMessageFromMonitor(curr_message)) {	
          //std::cout << "_recvMessage .GetNextMessageFromMonitor.suc!" <<std::endl;
          ...
          if ( i == byte_sequence_length ) {
              //std::cout << "_recvMessage.chq.payload check suc.break!!!" << std::endl;
              sick_message = curr_message;
              break;//break find loop //chq disable
          }
          else{
              std::cerr << "ERROR!!!_recvMessage.chq.payload check error!!!" << std::endl;///--///
              return false;//chq added
          }
       }// end if GetNextMessageFromMonitor
      /* Sleep a little bit */
      usleep(1000);     //sleep 1000us
      /* Check whether the allowed time has expired */
      gettimeofday(&end_time,NULL);
      unsigned int cur_waster_time = _computeElapsedTime(beg_time,end_time) ;
      if (cur_waster_time > timeout_value) {
           std::cerr << "ERROR!!!._recvMessage .cur check use time :" <<  cur_waster_time << " limit timeout_value : " << timeout_value <<std::endl;///--///
           return false;//chq add
          // throw SickTimeoutException();//chq disable
      }      
      
    }//end for (;;)
    //std::cout << "_recvMessage.last break suc!!!" << std::endl;
     return true;//chq add
  }
