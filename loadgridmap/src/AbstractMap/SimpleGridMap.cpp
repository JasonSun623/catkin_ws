...
#include "loadgridmap/AbstractMap/GridMapConstants.h"
#include "loadgridmap/AbstractMap/Geometry.h"
#include <fstream>
#include <string>
#include <iostream>
#include <memory.h>
...
void SimpleGridMap::LoadFromFile(std::string file_name)
{
  ...
  if(!ifs) {
    std::cerr<<"FATAL: map file not exist!!"<<std::endl;
  }
  std::cout << "load map file... " << std::endl;
  assert(ifs);
  map_name_ = file_name;
  std::cout << "load map file.map name: " << map_name_<<std::endl;
  std::string str;
  bool findData = false;
  while (ifs >> str) {
    if (str == "DATA") {
      std::cout << "load map file.find data! " <<std::endl;
      findData = true;
      break;
    }
  }
