...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(geometry_msgs::Twist& cmd_vel,
  ...
  ///加下面这句话 到点不旋转 chq
  ///angle =  limits.yaw_goal_tolerance;//force to stop when reached the goal firstly chq
  ///ROS_INFO("chq.LatchedStopRotateController::computeVelocityCommandsStopRotate.force angle <= limit roate!");///chq
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    cmd_vel.angular.z = 0.0;
    rotating_to_goal_ = false;
    ///ROS_INFO("chq.force angle <= limit roate!rotating_to_goal_ = false");///chq
  } else {
